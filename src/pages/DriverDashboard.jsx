...
export default function DriverDashboard({ profile, setProfile }) {
  ...
  const [driverRating, setDriverRating] = useState(profile.rating || null)
  ...
  async function fetchData() {
    const [{ data: reqs }, { data: bids }, { data: profileData }] = await Promise.all([
      ...
      supabase.from('bids').select('*, requests(*)')
        ...
        .order('created_at', { ascending: false })
        .limit(100),
      supabase.from('profiles').select('wallet_balance, is_online, rating, total_ratings, total_deliveries').eq('id', profile.id).single()
    ])
        ...
        <>
          <div style={{background:'linear-gradient(135deg, #7B1FA2, #9C27B0)', borderRadius:'12px', padding:'16px', marginBottom:'16px', color:'white'}}>
            ...
            <div style={{display:'flex', gap:'12px'}}>
              ...
              <div style={{flex:1, background:'rgba(255,255,255,0.15)', borderRadius:'10px', padding:'12px', textAlign:'center'}}>
                <div style={{fontSize:'22px', fontWeight:800, fontFamily:"'Baloo 2',cursive"}}>
                  {driverRating ? `⭐${parseFloat(driverRating).toFixed(1)}` : '⭐—'}
                </div>
                <div style={{fontSize:'11px', opacity:0.85}}>Rating</div>
              </div>
              <div style={{flex:1, background:'rgba(255,255,255,0.15)', borderRadius:'10px', padding:'12px', textAlign:'center'}}>
              ...
              <div style={{display:'flex', justifyContent:'space-between', alignItems:'center'}}>
                <div style={{display:'flex', gap:'10px', alignItems:'center'}}>
                  ...
                  <div>
                    ...
                    {bid.requests?.customer_rating && (
                      <div style={{fontSize:'13px', color:'#FFA726', fontWeight:600}}>
                        {'⭐'.repeat(bid.requests.customer_rating)} {bid.requests.customer_rating}/5 by customer
                      </div>
                    )}
                  </div>
                </div>
