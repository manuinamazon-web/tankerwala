      ...
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'requests' }, (payload) => {
        if (payload.new?.customer_id === profile.id) {
          ...
          if (payload.new?.status === 'pending' && payload.old?.status === 'accepted') {
            playSound(440, 0.4, 3)
            showNotification('⚠️ Driver cancelled delivery! Please choose another bid.')
          }
          fetchRequests()
        }
      })
        ...
        {req.status === 'pending' && (
          <div style={{display:'flex', gap:'8px'}}>
            <button
              ...
              style={{flex:1, padding:'12px', background:'#F0F4FF', border:'1.5px solid #C5D5F0', borderRadius:'10px', color:'#1565C0', fontWeight:600, fontSize:'14px', cursor:'pointer'}}
            >
              👁️ View Bids
            </button>
            <button
